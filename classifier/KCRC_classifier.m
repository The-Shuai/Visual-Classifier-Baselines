function [accuracy,acc_map] = KCRC_classifier(tr_max_fea,tr_label,ts_max_fea,ts_label,par)
beta             = par.method.param.beta;
...
Codes = (kernel_train+beta*eye(size(kernel_train,2)))\(kernel_tstr');
...
%     ker_train_class = kernel_train(tr_label == class, :);
%     rec_err(class,:) = diag(Codes'*kernel_train*Codes) - 2*diag(s'*ker_train_class*Codes) + diag((s'*ker_train)*s);
end
[~,ID] = min(rec_err);
ID = ID';
...
acc_map = zeros(nclass,nclass);
for jj = 1 : nclass
    idx_jj = find(ts_label == jj);
    curr_pred_label = ID(idx_jj);
    for kk = 1:nclass
        acc_map(kk,jj) = length(find(curr_pred_label == kk))/length(idx_jj);
    end
