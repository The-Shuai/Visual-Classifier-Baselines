function [accuracy,acc_map] = K_Euler_SRC_classifier(tr_max_fea,tr_label,ts_max_fea,ts_label,par)
acc_map = [];
alpha            = par.method.param.alpha;
delta            = par.method.param.delta;
...
% 
% 将样本映射到复数域
tr_max_fea = 2^(-1/2) * exp(delta * (pi * 1i) * tr_max_fea);
ts_max_fea = 2^(-1/2) * exp(delta * (pi * 1i) * ts_max_fea);
...
xxx=trace(kernel_test);
Codes = K_Euler_SRC_BCD(kernel_tstr',alpha,tr_max_fea,ts_max_fea);
